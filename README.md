Arnex Website
Sitio web corporativo desarrollado para Arnex, una empresa dedicada a procesos industriales y manufactura de arneses.

Sitio en línea
www.arnex.mx

Descripción
Este proyecto consiste en una landing page empresarial diseñada para presentar información sobre la empresa, sus procesos de trabajo y su identidad corporativa.
El objetivo del sitio es ofrecer una presencia digital clara, profesional y accesible para clientes y colaboradores.

Tecnologías utilizadas
HTML5
CSS3
JavaScript
GitHub Pages (hosting)

Características
Diseño limpio y profesional
Adaptado para dispositivos móviles (responsive)
Optimización de imágenes para mejorar la velocidad de carga
Publicación gratuita mediante GitHub Pages

Desarrollo
Desarrollado por Dimas como proyecto de desarrollo web para una empresa en crecimiento.

Licencia
Este proyecto fue creado con fines informativos para la empresa Arnex.
